...
export class MessageRepository {
  ...
  create(input: {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (input.replyToId) {
        ...
        if (!reply) throw new Error('The message being replied to was not found.');
      }
      ...
      const messageType = owned.some((attachment) => attachment.type === 'AUDIO')
        ? 'AUDIO'
        : owned.some((attachment) => attachment.type === 'IMAGE')
          ? 'IMAGE'
          : attachmentIds.length ? 'FILE' : 'TEXT';
      const created = await tx.message.create({
        data: {
          ...
          type: messageType,
          attachments: { connect: owned },
        },
        select: { id: true },
      });

      const recipients = participants.filter(({ userId }) => userId !== input.senderId);
  ...
  async markSeen(conversationId: string, userId: string) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      return { count: result.count, seenAt: now, participants };
    });
  }
}
